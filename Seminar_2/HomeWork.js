...
alert((num1 <= 1 && num2 >= 3) ? `Верно` : `Неверно`);
...
if (day <= 10) {
    ...
    alert(`${day} числонаходится в третьей декаде месяца`)
}
...
const countUnits = Math.round(number%10);
const countTens = Math.round(number/10%10);
const countHundred = Math.round(number/100%10);

console.log(`В числе ${number} количество сотен: ${countHundred}, десятков: ${countTens}, единиц: ${countUnits}`);
